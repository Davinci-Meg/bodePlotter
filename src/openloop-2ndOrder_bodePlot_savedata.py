...
with open('src/ax1_data.csv', 'w', newline='') as file:
    ...
    for freq, mag in zip(w/(2*np.pi), 20*np.log10(gain)):
        writer.writerow([freq, mag])

# Data for ax2
with open('src/ax2_data.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Frequency (Hz)', 'Phase (degrees)'])
    for freq, ph in zip(w/(2*np.pi), phase*180/np.pi):
        writer.writerow([freq, ph])
